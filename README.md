...
One container that lets several MCP clients work with [Bexio](https://www.bexio.com/) through the [Model Context Protocol](https://modelcontextprotocol.io/). Claude Desktop is one such client.
...
`docker-compose.yml` is an optional example. It publishes port 8000 and does not terminate TLS. See [docs/docker.md](docs/docker.md).
...
```
Authorization: Bearer bmg_the-access-key
```

The endpoint speaks [MCP Streamable HTTP](https://modelcontextprotocol.io/). How the client stores the URL and the header depends on that client.

Example for Claude Desktop, which starts a local process and does not call the URL itself:
...
- Original server: [promptpartner/bexio-mcp-server](https://github.com/promptpartner/bexio-mcp-server), building on the earlier server published at [bryner.tech](https://bryner.tech/).
